...
use App\Models\property_docs;
use App\Models\service_type;
use App\Models\vendors;
use App\Models\property_expenses;
use App\Models\expense_docs;
use App\Models\buyer;
use App\Models\property_sold;
...
class AdminController extends Controller
{
                        ...
                        if($sold_property){
                            ...
                            if($req->sale_type=="Lease"){
                                ...
                                if($add_invoice){
                                    ...
                                    return redirect::route('sold_property_detail',['sold_property_id'=>$sold_property->id])->with('success_msg',"Property Sold");
                                }else{
    ...
    public function submit_add_investor(Request $req)
    {
        ...
        if($validator->fails())
        {
            ...
            $user=investors::where('cnic',$req->cnic)->first();
            if(!$user){
                if($req->file('profile_image')!=null){
                    $profileImage=$req->file('profile_image')->store('Investors_Profile_Pictures');
                ...
                return redirect()->back()->with('error_msg', "Investor with given cnic already exits");
            }
        }
    }
    ...
    public function submit_edit_investor(Request $req)
    {
        ...
        if($validator->fails())
        {
            ...
            if($req->file('profile_image')!=null){
                $user=investors::where('id',$req->id)->first();
                if($user->profile_image!='' || $user->profile_image!=NULL){

                    $unlink_doc=Storage::delete($user->profile_image);
                }
                $profileImage=$req->file('profile_image')->store('Investors_Profile_Pictures');
    ...
    public function delete_property($id)
    {
        ...
        try{
            $unlink_doc=Storage::delete($property->property_image);
            $property->delete();
            if($property){
    ...
    public function delete_investor($id)
    {
        $investor=investors::where('id',$id)->first();
        try{
            $image=$investor->profile_image;
            $investor->delete();
            if($investor){
                $unlink_doc=Storage::delete($image);
    ...
    public function submit_reset_password(Request $req)
    {
        ...
        if($validator->fails())
        {
            ...
            if($req->c_pass==$req->new_pass){
                ...
                if($update_pass){
                    return redirect::to('/')->with('success_msg', " Password Changed Successfully");
                }else{
